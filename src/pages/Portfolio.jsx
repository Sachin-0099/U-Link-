...
const InteractivePortfolio = () => {
  ...
  return (
    <>
      <div className={`min-h-screen flex flex-col items-center justify-center p-4 md:p-8 relative overflow-hidden transition-colors duration-300 ${theme === "dark" ? "bg-gray-900 text-gray-100" : "bg-gray-50 text-gray-900"}`}>
        ...
        <div className="absolute top-0 left-0 right-0 flex justify-between items-center p-4 md:p-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button 
              ...
              className="md:hidden p-2 rounded-full transition-colors duration-300 hover:bg-opacity-20 hover:bg-gray-500"
        ...
        {isMenuOpen && (
          <motion.div 
            ...
            className="md:hidden absolute top-16 right-4 bg-white dark:bg-gray-800 shadow-lg rounded-md p-4 z-50"
          >
            <nav className="flex flex-col space-y-2">
              {Object.keys(portfolioData).map((key) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-md transition-all duration-300 text-left ${
                    activeSection === key 
                    ? "bg-[#b73235] text-white font-semibold" 
        ...
        {/* Main Content */}
        <div className="w-full max-w-7xl mx-auto mt-16 md:mt-0">
          {/* Team Selector - Desktop */}
          <div className="hidden md:flex justify-center mb-10 space-x-4">
            {Object.keys(portfolioData).map((key) => (
              <button
                key={key}
                ...
                className={`px-6 py-3 rounded-lg transition-all duration-300 ${
                  activeSection === key 
                  ? "bg-[#b73235] text-white font-semibold shadow-lg" 
                  : `${theme === "dark" ? "bg-gray-800 text-gray-300 hover:bg-gray-700" : "bg-white text-gray-700 hover:bg-gray-100"} shadow-md`
                }`}
              >
                {portfolioData[key].name}
              </button>
            ...
            <div className="space-y-8 order-2 lg:order-1">
              <motion.div variants={fadeIn}>
                <h1 className={`text-4xl md:text-5xl lg:text-6xl font-bold ${theme === "dark" ? "text-white" : "text-gray-900"} leading-tight`}>
                  Hi, I'm <span className="text-[#b73235]">{active.name.split(" ")[0]}</span>
                </h1>
              ...
              >
                <span className="text-[#b73235]">{active.title}</span> 
                <span 
              ...
              <motion.div variants={fadeIn} className="flex space-x-4">
                {active.social && (
                  <>
                    <a href={active.social.linkedin} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-[#b73235] hover:text-white transition-colors duration-300">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                      </svg>
                    </a>
                    <a href={active.social.twitter} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-[#b73235] hover:text-white transition-colors duration-300">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                      </svg>
                    </a>
                    <a href={active.social.github} target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:bg-[#b73235] hover:text-white transition-colors duration-300">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <motion.div variants={fadeIn} className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 pt-4">
                ...
                <button
                  onClick={() => {
                    window.location.href = `mailto:${active.email}?subject=Let's Connect&body=Hi ${active.name.split(" ")[0]},%0D%0A%0D%0AI'm reaching out to you. It would be great to connect and explore potential collaboration opportunities.%0D%0A%0D%0ABest regards,%0D%0A[Your Name]`;
                  }}
